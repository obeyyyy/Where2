...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-sm border-b border-gray-100 h-24 md:h-28">
      <nav className="container mx-auto px-6 flex items-center h-full justify-between relative">
        ...
        {/* Desktop Nav Links */}
        <ul className="hidden md:flex gap-8 font-semibold text-gray-700 text-base items-center ml-8">
          {navLinks.map(link => (
            <li key={link.href}>
              <a href={link.href} className="hover:text-[#FF8C00] focus:text-[#FF8C00] transition-colors px-2 py-1 rounded focus:outline-none focus:ring-2 focus:ring-[#FFA500]" tabIndex={0}>{link.label}</a>
            </li>
          ))}
        </ul>
        {/* Mobile Hamburger */}
        ...
        >
          <svg className="w-8 h-8" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d={menuOpen ? "M6 18L18 6M6 6l12 12" : "M4 8h16M4 16h16"} /></svg>
        </button>
        {/* Mobile Menu */}
        {menuOpen && (
          <ul className="absolute top-full left-0 right-0 bg-white/95 border-b border-gray-100 shadow-lg flex flex-col items-center gap-4 py-6 md:hidden animate-fadeIn">
            {navLinks.map(link => (
              <li key={link.href}>
                <a href={link.href} className="block px-4 py-2 font-semibold text-gray-700 hover:text-[#FF8C00] focus:text-[#FF8C00] focus:outline-none focus:ring-2 focus:ring-[#FFA500] rounded text-lg" tabIndex={0} onClick={() => setMenuOpen(false)}>{link.label}</a>
              </li>
            ))}
          </ul>
        )}
